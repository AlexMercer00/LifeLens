...
export async function searchCompounds(query: string): Promise<Compound[]> {
  try {
    ...
    return response.data.PC_Compounds.map((compound: any) => ({
      ...
      formula: compound.props.find((p: any) => p.urn.label === 'Molecular Formula')?.value?.sval || undefined,
      molecularWeight: compound.props.find((p: any) => p.urn.label === 'Molecular Weight')?.value?.fval || undefined,
    }));
  } catch (error) {
